...
<template>
  <NuxtLink :exact-active-class="'[--opacity:1]'" :to="to" :style="{
    '--color': `hsl(${hslColor.h} ${hslColor.s * 100}% ${hslColor.l * 100}% / var(--opacity, 1))`,
    '--textColor': textColor,
    '--opacity': 0.5,
  }" class="inline-block bg-[var(--color)] rounded rounded-full font-medium tracking-wider uppercase text-12px px-2 py-1">
    <slot />
  </NuxtLink>
</template>
