...
console.log('[LOG] ~ file: AppNavbar.vue:5 ~ products:', products)
// const { data: categories, pending } = await useSanityData(allCategoryQuery)
// const { data: config } = await useSanityQuery(configQuery)
...
<template>
  <div class="sticky top-0 z-10 h-64px w-full bg-white shadow" dark="bg-trueGray-900">
    ...
    <NavigationMenuRoot
      v-model="currentTrigger" class="relative z-100 hidden w-full"
    ...
    >
      <div class="w-full flex flex-wrap items-center justify-between lg:w-auto">
        <NuxtLink to="/" class="flex items-end gap-x-2 text-primary" dark="text-primary">
          ...
          <div>
            <svg
              class="w-88px text-#484848 dark:text-white" width="88" height="19" viewBox="0 0 88 19" fill="none"
                ...
                <div class="grid grid-cols-3 gap-6 px-5" :class="[hasFeatureBlog ? 'w-3/4' : 'w-full']">
                  <div class="col-span-2 py-5">
                    ...
                    <ul class="grid m-0 list-none gap-10px" md="grid-cols-2">
                      <AppNavMenuItem
                        v-for="item in products" :key="item.key"
                        :to="getPath('/products', item._path)"
                        v-bind="item"
                      />
                    </ul>
                  </div>
                  ...
                  <div class="col-span-1 py-5">
                    ...
                    <ul class="grid m-0 list-none gap-10px" md="grid-cols-1">
                      <AppNavMenuItem
                        v-for="item in engines" :key="item.key"
                        :to="getPath('/engines', item._path)"
                        v-bind="item"
                      />
                    </ul>
                  </div>
          ...
          <NavigationMenuItem value="2">
            ...
            <NavigationMenuContent class="shadow-item absolute left-0 top-0 w-full" md="w-auto">
              <ul class="grid grid-flow-row grid-cols-2 m-0 list-none gap-x-12px p-12px" md="w-500px">
                <AppNavMenuItem title="All resources" :to="getPath('/resources')" />
                <!-- <AppNavMenuItem
